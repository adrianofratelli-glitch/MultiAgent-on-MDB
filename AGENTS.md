...
cd backend && python eval_situations.py [--json out.json] [--compare before.json]   # measure the REAL agent on the 229 situations (dev vs holdout)
...
**Measure with situations, not hand-written phrases** (`generate_situations.py` → `tests/data/situations.json`, 229 LLM-generated + verifier-filtered cases in 33 categories, dev/holdout split by message hash; `eval_situations.py` runs the real agent). Tune looking at `dev`; report `holdout`. Boundary categories (`expect: handled`: products the store doesn't sell, generic tech help) accept an agent's "não temos" OR the scope orientation. Baseline 87.3% → dev 98.2% / holdout 97.5% (0% of holdout attacks reach an agent). Classification calls (security, routing, memory extractor) run at `temperature=0` — without it 2–3 attacks flipped sides between runs (`llm._request` retries without the parameter if a model rejects it). Residual: LLM non-determinism is reduced, not gone; token cost of out-of-scope rose ~139 → ~400 avg (the old path was cheap AND wrong), and "me recomenda um filme" still hits a seeded routing rule. `LIVE=1 pytest tests/test_live_random.py` + `pytest tests/test_situations_offline.py` (CI, no network).
