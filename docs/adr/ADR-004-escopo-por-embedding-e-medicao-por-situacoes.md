...
- **"Me recomenda um filme"** ainda vai ao `product_agent`: `recomenda` é palavra-chave de uma regra de roteamento seedada (dado, não código). Registrado como limitação; não reescrevi a regra para não mexer no dataset de roteamento.
- **Amostras**: o dev tem só 6 itens `in` sem palavra-chave (a maioria das mensagens legítimas tem palavra-chave e nunca chega ao classificador). Por isso o piso de 0,04. Recalibrar quando o conjunto crescer.
