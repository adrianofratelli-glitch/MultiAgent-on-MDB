...
**Out-of-scope questions** ("qual é a temperatura hoje?"): `router.has_domain_signal` (strong vocabulary) / `has_weak_signal` (generic words like "conta", "ajuda" — never enough alone) decide. No domain signal → canned orientation from `guidance.out_of_scope_reply`: **0 tokens** (the security LLM classifier is skipped too, unless `looks_like_instruction` sees an injection wrapped in the question), no agent, no cache, timeline event `guardrail` with `result.out_of_scope=true` (UI card "🧭 Guardrail de escopo"), and it is NOT treated as an attack. Weak-only → the orchestrator LLM decides; if it can't, the answer is the orientation, never an `order_agent` guess. Greetings ("bom dia, tudo bem?") and capability questions ("o que você sabe fazer?") get the welcome. Mixed messages (one sentence in scope, another not) answer the store part and append what was left out (`out_of_scope_sentences`; never cached). Battery: `LIVE=1 pytest tests/test_live_random.py`.
...
`main` está publicado (`352cd6c`). Suíte offline verde (294, sem rede), eval dourado **29/29 em modo LIVE**, baterias live dos 4 clientes e de perguntas aleatórias verdes. Não refaça as decisões já registradas nos ADRs 002/003 — elas vieram de medição contra o cluster, não de preferência.
