...
`main` está publicado até `00c89d6` (22/09/2026: resiliência por padrão, caos, isolamento de banco, graphify) — push verde com o hook `pre-push` completo (ruff + 408 offline + `test_live.py` 7/7). ADR-004 documenta a rodada anterior.
...
**Recuperação em PC novo (o que está e o que NÃO está no GitHub):** o repositório tem código, testes, docs/ADRs, `tests/data/situations.json`, `.claude/rules` + `settings.json` e o essencial do graphify (`graphify-out/graph.json`, `GRAPH_REPORT.md`, `manifest.json`, `cost.json`, rótulos; sem caminhos absolutos — `graphify update` continua de onde parou; o cache e o HTML são regenerados). **NÃO está no git, de propósito:** `.env` (segredos: Mongo, Anthropic, Grove, Langfuse, JWT — guarde num cofre de senhas; `.env.example` lista as variáveis que o app lê), `.venv` (`python3.12 -m venv .venv && pip install -r backend/requirements.txt`), `.claude/settings.local.json` (permissões pessoais) e `../STATUS_PORTFOLIO.md` (o workspace pai não é um repositório). O estado do Atlas vive no cluster, não no PC (lista acima). Passos: clonar → `git config core.hooksPath .githooks` → venv → `cp .env.example .env` e preencher os segredos → confirmar o cluster ligado (`DataStore.connect`) → `cd backend && pytest -q`.
